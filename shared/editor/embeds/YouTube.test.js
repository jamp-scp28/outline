"use strict";

require("core-js/modules/es.regexp.exec.js");

require("core-js/modules/es.string.match.js");

var _YouTube = _interopRequireDefault(require("./YouTube"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

describe("YouTube", function () {
  var match = _YouTube.default.ENABLED[0];
  test("to be enabled on video link", function () {
    expect("https://www.youtube.com/watch?v=dQw4w9WgXcQ".match(match)).toBeTruthy();
  });
  test("to be enabled on embed link", function () {
    expect("https://www.youtube.com/embed?v=dQw4w9WgXcQ".match(match)).toBeTruthy();
  });
  test("to be enabled on shortlink", function () {
    expect("https://youtu.be/dQw4w9WgXcQ".match(match)).toBeTruthy();
  });
  test("to not be enabled elsewhere", function () {
    expect("https://youtu.be".match(match)).toBe(null);
    expect("https://youtube.com".match(match)).toBe(null);
    expect("https://www.youtube.com".match(match)).toBe(null);
    expect("https://www.youtube.com/logout".match(match)).toBe(null);
    expect("https://www.youtube.com/feed/subscriptions".match(match)).toBe(null);
  });
});