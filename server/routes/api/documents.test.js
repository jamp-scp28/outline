...
describe("#documents.info", () => {
  ...
  it("should not return document of a deleted collection, when the user was absent in the collection", async () => {
    ...
    const doc = await (0, _factories.buildDocument)({
      collectionId: collection.id,
      teamId: user.teamId,
      userId: user.id
    });
  ...
  describe("apiVersion=2", () => {
    it("should return sharedTree from shareId", async () => {
      ...
      expect(body.data.sharedTree).toEqual((_collection$documentS = collection.documentStructure) === null || _collection$documentS === void 0 ? void 0 : _collection$documentS[0]);
      await share.reload();
      expect(share.lastAccessedAt).toBeTruthy();
    });
...
describe("#documents.search", () => {
  ...
  it("should return results using shareId", async () => {
    const findableDocument = await (0, _factories.buildDocument)({
      title: "search term",
      text: "random text"
    });
    await (0, _factories.buildDocument)({
...
describe("#documents.unpublish", () => {
  it("should unpublish a document", async () => {
    ...
    const reloaded = await _models.Document.unscoped().findByPk(document.id);
