...
describe("#team.update", () => {
  ...
  it("should add new allowed Domains, removing empty string values", async () => {
    ...
    const teamDomains = await _models.TeamDomain.findAll({
      where: {
        teamId: team.id
      }
    });
    expect(teamDomains.map(d => d.name)).toEqual(["example.com", "example.org"]);
  });
  it("should remove old allowed Domains", async () => {
    ...
    const existingTeamDomain = await _models.TeamDomain.create({
      teamId: team.id,
      name: "example.com",
    ...
    expect(await _models.TeamDomain.findByPk(existingTeamDomain.id)).toBeNull();
  });
  it("should add new allowed domains and remove old ones", async () => {
    ...
    expect(teamDomains.map(d => d.name).sort()).toEqual(["example.org", "example.net"].sort());
    expect(await _models.TeamDomain.findByPk(existingTeamDomain.id)).toBeNull();
  });
