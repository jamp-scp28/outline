      ...
      case "documents.delete":
        {
          ...
          return socketio.to(`collection-${document.collectionId}`).emit("entities", {
            event: event.name,
            documentIds: [{
              id: document.id,
              updatedAt: document.updatedAt
      ...
      case "collections.create":
        {
          ...
          socketio.to(collection.permission ? `team-${collection.teamId}` : `collection-${collection.id}`).emit("entities", {
            event: event.name,
            collectionIds: [{
              id: collection.id,
          ...
          return socketio.to(collection.permission ? `team-${collection.teamId}` : `collection-${collection.id}`).emit("join", {
            event: event.name,
      ...
      case "collections.remove_user":
        {
          ...
          if (membershipUserIds.includes(event.userId)) {
            ...
            socketio.to(`collection-${event.collectionId}`).emit("collections.remove_user", {
              event: event.name,
              userId: event.userId,
              collectionId: event.collectionId
      ...
      case "collections.add_group":
        {
          ...
          for (const groupMembership of group.groupMemberships) {
            socketio.to(`user-${groupMembership.userId}`).emit("collections.add_user", {
              event: event.name,
      ...
      case "collections.remove_group":
        {
          ...
          for (const groupMembership of group.groupMemberships) {
            if (membershipUserIds.includes(groupMembership.userId)) {
              ...
              socketio.to(`user-${groupMembership.userId}`).emit("collections.remove_user", {
                event: event.name,
                userId: groupMembership.userId,
                collectionId: event.collectionId
      ...
      case "fileOperations.update":
        {
          ...
          const data = await (0, _presenters.presentFileOperation)(fileOperation);
      ...
      case "groups.add_user":
        {
          ...
          for (const collectionGroup of collectionGroupMemberships) {
            ...
            socketio.to(`collection-${collectionGroup.collectionId}`).emit("collections.add_user", {
              event: event.name,
              userId: event.userId,
              collectionId: collectionGroup.collectionId
            }); // tell any user clients to connect to the websocket channel for the collection
      ...
      case "groups.remove_user":
        {
          ...
          for (const collectionGroup of collectionGroupMemberships) {
            ...
            if (hasMemberships) {
              ...
              // let everyone with access to the collection know a user was removed
              socketio.to(`collection-${collectionGroup.collectionId}`).emit("collections.remove_user", {
                event: event.name,
                userId: event.userId,
                collectionId: collectionGroup.collectionId
              }); // tell any user clients to disconnect from the websocket channel for the collection
