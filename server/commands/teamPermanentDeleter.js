...
async function teamPermanentDeleter(team) {
  ...
  _Logger.default.info("commands", `Permanently deleting team ${team.name} (${team.id})`);
  ...
  try {
    ...
    await _models.Attachment.findAllInBatches({
      ...
      _Logger.default.info("commands", `Deleting attachments ${options.offset} – ${(options.offset || 0) + ((options === null || options === void 0 ? void 0 : options.limit) || 0)}…`);
    ...
    await _models.NotificationSetting.destroy({
      where: {
        teamId
      },
      force: true,
